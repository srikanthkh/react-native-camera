...
export default class Camera extends Component {
    ...
    capture(options) {
        ...
        if (options.mode === Camera.constants.CaptureMode.video) {
            ...
            console.log('video mode:', this.state.isRecording);
        }
    ...
    deletePicture(imagePath) {
        console.log('imagePath123', imagePath);

        return CameraManager.deletePicture(imagePath);
    }

    startAudioRecording() {
        if(!this.state.isAudioRecording) {
            this.setState({isAudioRecording: true});
            return CameraManager.startAudioRecording();
        }
    ...
    stopAudioRecording() {
        if (this.state.isAudioRecording) {
            ...
            CameraManager.stopAudioRecording();
            return CameraManager.stopAudioRecording();
        }
